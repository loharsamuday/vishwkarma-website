<?php
// idioms-phrasal-verbs/phrasal-verbs.php
session_start();
require_once '../config/database.php';
require_once '../includes/functions.php';

$page_title = "Important Phrasal Verbs for Competitive Exams";
$seo_desc = "Learn important English phrasal verbs with Hindi meaning, examples, and memory tricks for SSC, Bank, and other competitive exams.";
...
$stmt_total = $pdo->prepare("SELECT COUNT(*) FROM phrasal_verbs p WHERE $where_sql");
$stmt_total->execute($params);
$total_records = $stmt_total->fetchColumn();
$total_pages = ceil($total_records / $limit);

$sql = "SELECT p.*, c.name as category_name FROM phrasal_verbs p LEFT JOIN exam_categories c ON p.category_id = c.id WHERE $where_sql ORDER BY p.featured DESC, p.id DESC LIMIT $limit OFFSET $offset";
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2">
                ...
                <li class="breadcrumb-item active" aria-current="page">Phrasal Verbs</li>
            </ol>
        </nav>
        <h1 class="fw-bold">Important Phrasal Verbs</h1>
        <p class="text-muted mb-0">Master English phrasal verbs easily with Hindi meaning and memory tricks.</p>
    </div>
</div>
    ...
    <div class="row">
                ...
                <div class="card-body">
                    <form action="" method="GET">
                        ...
                        <?php if($category_id || $difficulty): ?>
                            <a href="phrasal-verbs.php" class="btn btn-sm btn-outline-secondary w-100">Clear Filters</a>
                        <?php endif; ?>
                    </form>
                </div>
            </div>
        ...
        <div class="col-lg-9">
            <?php if(count($phrasal_verbs) > 0): ?>
                <div class="row g-4">
                    <?php foreach($phrasal_verbs as $item): ?>
                        <div class="col-md-6">
                            <div class="card h-100 border-0 shadow-sm pv-card transition-hover border-start border-success border-4">
                                <div class="card-body">
                        ...
                        <ul class="pagination justify-content-center">
                            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                    <a class="page-link text-success" href="?page=<?= $i ?>&category=<?= $category_id ?>&difficulty=<?= urlencode($difficulty) ?>"><?= $i ?></a>
                                </li>
                            <?php endfor; ?>
                        </ul>
                    </nav>
            ...
            <?php else: ?>
                <div class="alert alert-info text-center py-5">
                    ...
                    <h4>No Phrasal Verbs Found</h4>
                    <p>Try adjusting your filters or check back later for new content.</p>
                    <a href="phrasal-verbs.php" class="btn btn-outline-success">View All</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
<style>
.pv-card.transition-hover {
    transition: transform 0.2s ease, box-shadow 0.2s ease;
}
.pv-card.transition-hover:hover {
